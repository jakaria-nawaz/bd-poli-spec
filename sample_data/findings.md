# Bam-Dan Survey Key Findings
Generated: 2026-04-18
Sample: 17259 responses | Apr 3-Apr 17 2026 | bam-dan.com

## IMPORTANT: Sample Caveat
Self-selected online sample. 88.7% male, 69.0% students.
These findings describe Bangladesh's politically engaged online youth,
not the general population.

## 1. The Centrist Illusion
52.4% of algorithm-classified Centrists have a
negative (left-of-centre) economic score.

## 2. Biggest Gender Gap
Q16 (Equal property rights for men and women):
  Male mean = -0.23, Female mean = 0.59
  Gap = 0.82 — largest gender difference in the dataset.

## 3. Most Agreed Question
Q17 - "Strict rules & fines for factories to protect environment"
  Mean response: 1.40 (near-universal agreement)

## 4. Most Divisive Question
Q9 - "Laws should follow religious rules"
  Standard deviation: 1.48 — bimodal distribution,
  two strong opposing camps.

## 5. Generation Surprise
Q20 (Strictly ban anything hurting religious sentiments):
  18-24 age group mean = 1.23
  35-44 age group mean = 0.74
  Younger respondents are stricter on religious speech.

## 6. Education and Minority Rights
Q21 (Special rights for indigenous/ethnic minorities):
  Below SSC mean = 0.30
  Bachelor/Masters mean = 1.08
  Education is the strongest predictor of support for minority rights.

## 7. Probashi Difference
Q20 (Strictly ban anything hurting religious sentiments):
  Bangladesh respondents mean = 1.17
  Probashi (abroad) mean = 0.89
  Difference = -0.28

## 8. Growth Story
Peak day: Apr 10 (12798 responses in one day)
Total responses: 17259 over 13 days
Average per day: 1328
